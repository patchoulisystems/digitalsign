...
$.datepick.newDate(2014, 12, 25) */
    ...
    _keyDown: function (event) {
      ...
      if (inst.inline || inst.div) {
        if (event.keyCode === 9) {
          ...
          for (var key in inst.options.commands) {
            if (inst.options.commands.hasOwnProperty(key)) {
              command = inst.options.commands[key]; // Dislikes !!
              /* jshint -W018 */ if (
        ...
        command = inst.options.commands.current; // Dislikes !!
        /* jshint -W018 */ if (
          command.keystroke.keyCode === event.keyCode &&
    ...
    _generateMonthSelection: function (
      ...
      if (yearRange === "any") {
        ...
        if (start.getTime() < end.getTime()) {
          start = (minDate && minDate.getTime() > start.getTime()
            ? minDate
            : start
          ).getFullYear();
          ...
          start = (maxDate && maxDate.getTime() < start.getTime()
            ? maxDate
            : start
          ).getFullYear();
